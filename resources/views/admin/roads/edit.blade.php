...
<div class="modal" style="width: 100%; max-width: 1200px; margin: 0 auto; box-shadow: none;">
    <div class="modal-body" style="padding: 0;">
        <form action="{{ route('admin.roads.update', $road->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="form-grid">
                <div class="form-group full-width">
                    <label for="nama">Nama Jalan <span style="color: #ef4444">*</span></label>
                    <input type="text" id="nama" name="nama" class="form-input @error('nama') error @enderror" value="{{ old('nama', $road->nama) }}" required />
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="panjang" name="panjang" class="form-input" step="0.1" min="0" value="{{ old('panjang', $road->panjang) }}" required />
                </div>
                <div class="form-group">
                    <label for="lebar">Lebar (m) <span style="color: #ef4444">*</span></label>
                    <input type="number" id="lebar" name="lebar" class="form-input" step="0.1" min="0" value="{{ old('lebar', $road->lebar) }}" required />
                </div>
                <div class="form-group">
                    <label for="jenis_perkerasan">Jenis Perkerasan</label>
                    <select id="jenis_perkerasan" name="jenis_perkerasan" class="form-select">
                        <option value="Aspal" {{ old('jenis_perkerasan', $road->jenis_perkerasan) == 'Aspal' ? 'selected' : '' }}>Aspal</option>
                        <option value="Beton" {{ old('jenis_perkerasan', $road->jenis_perkerasan) == 'Beton' ? 'selected' : '' }}>Beton</option>
                        <option value="Tanah" {{ old('jenis_perkerasan', $road->jenis_perkerasan) == 'Tanah' ? 'selected' : '' }}>Tanah</option>
                        <option value="Kerikil" {{ old('jenis_perkerasan', $road->jenis_perkerasan) == 'Kerikil' ? 'selected' : '' }}>Kerikil</option>
                        <option value="Paving" {{ old('jenis_perkerasan', $road->jenis_perkerasan) == 'Paving' ? 'selected' : '' }}>Paving</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="kondisi">Kondisi Jalan</label>
                    <select id="kondisi" name="kondisi" class="form-select">
                        <option value="Baik" {{ old('kondisi', $road->kondisi) == 'Baik' ? 'selected' : '' }}>Baik</option>
                        <option value="Sedang" {{ old('kondisi', $road->kondisi) == 'Sedang' ? 'selected' : '' }}>Sedang</option>
                        <option value="Rusak Ringan" {{ old('kondisi', $road->kondisi) == 'Rusak Ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                        <option value="Rusak Berat" {{ old('kondisi', $road->kondisi) == 'Rusak Berat' ? 'selected' : '' }}>Rusak Berat</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="kecamatan">Kecamatan <span style="color: #ef4444">*</span></label>
                    <input type="text" id="kecamatan" name="kecamatan" class="form-input" value="{{ old('kecamatan', $road->kecamatan) }}" required />
                </div>
                <div class="form-group">
                    <label for="kelurahan">Kelurahan</label>
                    <input type="text" id="kelurahan" name="kelurahan" class="form-input" value="{{ old('kelurahan', $road->kelurahan) }}" />
                </div>
                <div class="form-group">
                    <label for="tahun">Tahun Pembangunan</label>
                    <input type="number" id="tahun" name="tahun" class="form-input" value="{{ old('tahun', $road->tahun) }}" min="1900" max="2099" />
                </div>
                
                @php
                    $initialCoords = is_array($road->coordinates) ? $road->coordinates : [];
                @endphp
                <div class="form-group">
                    <label for="latitude">Latitude (Garis Lintang)</label>
                    <input type="text" id="latitude" class="form-input" placeholder="Contoh: -0.02" value="{{ count($initialCoords) > 0 ? $initialCoords[0][0] : '' }}" />
                </div>
                <div class="form-group">
                    <label for="longitude">Longitude (Garis Bujur)</label>
                    <input type="text" id="longitude" class="form-input" placeholder="Contoh: 109.34" value="{{ count($initialCoords) > 0 ? $initialCoords[0][1] : '' }}" />
                </div>
                
                <!-- Hidden input for coordinate json array -->
                <input type="hidden" id="coordinates_json" name="coordinates_json" value="{{ old('coordinates_json', json_encode($initialCoords)) }}">
            ...
            <div style="margin-top:20px; text-align:right;">
                ...
                <button type="submit" class="btn btn-primary" style="margin-left: 10px;"><i class="fas fa-save"></i> Perbarui Jalan</button>
            </div>
        </form>
    </div>
</div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const drawControl = new L.Control.Draw({
            ...
            draw: {
                ...
                marker: true // Point markers for roads
            }
        });
        ...
        if(existingData && existingData !== '[]') {
            ...
            if(lastMarker) {
                modalMap.setView([coords[0][0], coords[0][1]], 16);
            }
        }
